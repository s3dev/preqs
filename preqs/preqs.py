...
class Requirements:
    ...
    def __init__(self):
        ...
        self._files = None              # List of Python modules to be analysed.
    ...
    def _clean_imports(self) -> bool:
        ...
        if self._imps & _locals & _builtins:
    ...
    def _find_files(self) -> bool:
        ...
        files = list(filter(lambda x: os.path.dirname(x) not in self._IGNORE_DIRS, tmp))
        if self._args.debug:
    ...
    def _get_installed_version(self) -> bool:
        ...
        handled and ignored. This is OK.
    ...
    def _write(self) -> bool:
        ...
        if self._args.print:
            ...
            content = '\n'.join(f'{pkg}=={ver}' for pkg, ver in _reqs) + '\n'
            with open(self._ofile, 'w', encoding='utf-8') as f:
                f.write(content)
            if not self._compare(content=content):
